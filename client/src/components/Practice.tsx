...
function Image({index, num} : {num: number, index:number}) {
        ...
        return (
            ...
            >
                <img src={posters[(index-1) % 3]} alt="Kill Bill" className='rounded-xl border-3 border-white'/>
            </div> 
        )
    }
...
export default function Practice({n} : {n: number}) {
    return (
        <div className="m-auto w-8/10 border- border-white min-h-full relative grid grid-rows-6 grid-cols-6 *:borde">
            {
